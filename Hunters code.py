...
START_DATE      = "2026-04-25" # YYYY-MM-DD
END_DATE        = "2026-06-16" # YYYY-MM-DD

ACCOUNT_BALANCE = 10_000       # Starting balance per system
FIXED_NOTIONAL  = None          # Fixed $ per trade (set to None to use full account)
...
def run_system(df, ma_period, tp_version):
    ...
    while i < n - 1:
        ...
        # ── stop loss: first prior candle in lookback that crosses the MA ────
        stop_price = None
        start_idx = max(0, i - SL_LOOKBACK)
        if direction == "long":
            for j in range(start_idx, i):
                if closes[j] < mas[j] and dates[j] == dates[i]:
                    stop_price = closes[j]
                    break
        else:
            for j in range(start_idx, i):
        ...
        # ── take profit (Version 1: high/low before pullback) ─
        ...
        # TP = high from the candle right before the pullback started (long)
        #    = low  from the candle right before the pullback started (short)
        tp_price = None
        # New TP rule: for "swing" TP version set take-profit to be
        # 2x the risk above/below the entry (i.e. 2 * (entry - stop)).
        if tp_version == "swing":
            # risk already computed as absolute distance between entry and stop
            if direction == "long":
